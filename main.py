...
notes = {
    "Ласкаво просимо!" : {
        "текст" : "Це найкращий додаток для нотаток у світі!",
...
button_note_create = QPushButton('Створити нотатку') # з'являється вікно з полем "Введіть ім'я нотатки"
...
def del_note():
    if list_notes.selectedItems():
        ...
        print("Нотатка для вилучення не обрана!")



'''Работа з тегами нотатки'''
def add_tag():
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        tag = field_tag.text()
        ...
        print("Нотатка для додавання тега не обрана!")
